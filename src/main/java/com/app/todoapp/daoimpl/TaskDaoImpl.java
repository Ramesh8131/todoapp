...
public class TaskDaoImpl implements ITaskDao {
	...
	public TodoTask getTaskByTitle(String title) {
		try {
			TodoTask todoTask = (TodoTask) entityManager.createQuery("from TodoTask where title ='" + title + "'").getSingleResult();
			return todoTask;
	...
	public List<TodoTask> getSearchTaskByTitle(String title, int startIndex, int maxPageSize) {
		List<TodoTask> user = (List<TodoTask>) entityManager
				.createQuery("from TodoTask where status = 1 and title like '%" + title + "%'").setFirstResult(startIndex)
				.setMaxResults(maxPageSize).getResultList();
		return user;
	}
	...
	public TodoTask getTaskByTitleId(String title, int id) {
		try {
			TodoTask todoTask = (TodoTask) entityManager.createQuery("from TodoTask where title ='" + title + "' and id!="+id).getSingleResult();
			return todoTask;
		} catch (NoResultException e) {
			return null;
		} catch (Exception e) {
